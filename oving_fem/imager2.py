...
def ptest1(fid1='images/northernlights.jpeg', fid2="images/einstein.jpeg",steps=5,newsize=250):
    im1 = Imager(fid1); im2 = Imager(fid2)
    im1 = im1.resize(500,500); im2 = im2.resize(500,500)
    mor = im1.morph(im2, 0.7)
    concat = mor.concat_horiz(mor)
    concat.display()
    #roll = im1.morphroll(im2,steps=steps)
    #roll.display()
    return concat

def ptest2(fid1='images/einstein.jpeg',outfid='images/library.jpeg',levels=2,newsize=250,scale=0.5):
    im1 = Imager(fid1);
    im1 = im1.resize(500,500);
    im2 = im1.mortun(im1, levels = levels ,scale = scale)

    im2.display()
    #im2.dump_image(outfid)
    return im2

def ptest3(fid1='images/kdfinger.jpeg', fid2="images/einstein.jpeg",newsize=250,levels=1,scale=0.50):
    im1 = Imager(fid1); im2 = Imager(fid2)
    im1 = im1.resize(500,500); im2 = im2.resize(500,500)
    box = im1.morph4(im2)
    box.display()
